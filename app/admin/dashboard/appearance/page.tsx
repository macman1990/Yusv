...
export default function AppearancePage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await adminApi.updateSetting('theme', theme);
      setAccentColor(theme.accent_color);
      localStorage.setItem('accentOverride', theme.accent_color);
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="rounded-xl border border-border/40 p-5 space-y-4">
          ...
          <div>
            ...
            <Select value={theme.default_theme} onValueChange={(v) => setTheme({ ...theme, default_theme: v })}>
        ...
        <div className="rounded-xl border border-border/40 p-5 space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {['#e8b339', '#3b82f6', '#10b981', '#ef4444', '#f97316', '#ec4899'].map(color => (
              <button
                key={color}
                onClick={() => setTheme({ ...theme, accent_color: color })}
        ...
        <div className="rounded-xl border border-border/40 p-5 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch checked={theme.animation_enabled} onCheckedChange={(v) => setTheme({ ...theme, animation_enabled: v })} />
          </div>
          <div className="flex items-center justify-between">
            <Label>Custom Cursor</Label>
            <Switch checked={theme.cursor_enabled} onCheckedChange={(v) => setTheme({ ...theme, cursor_enabled: v })} />
          </div>
          <div className="flex items-center justify-between">
            <Label>Parallax Effects</Label>
            <Switch checked={theme.parallax_enabled} onCheckedChange={(v) => setTheme({ ...theme, parallax_enabled: v })} />
          </div>
          <div>
            <Label className="mb-1.5 block">Animation Intensity</Label>
            <Select value={theme.animation_intensity} onValueChange={(v) => setTheme({ ...theme, animation_intensity: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            <Label className="mb-1.5 block">Transition Speed: {theme.transition_speed}s</Label>
            <input
              type="range"
              min="0.2"
              max="1.5"
              step="0.1"
              value={theme.transition_speed}
              onChange={(e) => setTheme({ ...theme, transition_speed: parseFloat(e.target.value) })}
              className="w-full"
            />
          </div>
        </div>
